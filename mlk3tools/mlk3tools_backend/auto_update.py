"""Auto-update utilities for the MLK3Tools backend."""
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        if new_version:
            return f"MLK3Tools updated to {new_version}. Please restart Steam."
        return "MLK3Tools update applied. Please restart Steam."
...
def _fetch_github_latest(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            proxy_url = "https://mlk3tools.vercel.app/api/github-latest"
    ...
    if not zip_url and tag_name:
        zip_url = f"https://mlk3tools.vercel.app/api/get-plugin/{tag_name}"
...
def check_for_update_once() -> str:
    ...
    try:
        ...
        return f"MLK3Tools updated to {latest_version}. Please restart Steam."
...
def _check_and_donate_keys() -> None:
    ...
    try:
        ...
        if not steam_path:
            logger.warn("MLK3Tools: Cannot donate keys - Steam path not found")
            return
        ...
        if pairs:
            ...
            logger.log("MLK3Tools: No valid keys found to donate")
            
    except Exception as exc:
        logger.warn(f"MLK3Tools: Donate keys check failed: {exc}")
...
def restart_steam_internal() -> bool:
    ...
    if not os.path.exists(script_path):
        logger.error(f"MLK3Tools: restart script not found: {script_path}")
        return False
    try:
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("MLK3Tools: Restart script launched (hidden)")
        return True
    except Exception as exc:
        logger.error(f"MLK3Tools: Failed to launch restart script: {exc}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"MLK3Tools: CheckForUpdatesNow failed: {exc}")
