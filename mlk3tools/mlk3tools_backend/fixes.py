...
FIXES_INDEX_URL = "https://index.mlk3tools.work/fixes-index.json"
...
def _fetch_fixes_index() -> Optional[Dict]:
    ...
    try:
        client = ensure_http_client("MLK3Tools: FixesIndex")
        ...
        if resp.status_code == 200:
            ...
            logger.log(f"MLK3Tools: Fixes index loaded ({len(generic_set)} generic, {len(online_set)} online)")
            return index
        else:
            logger.warn(f"MLK3Tools: Fixes index fetch returned {resp.status_code}")
    except Exception as exc:
        logger.warn(f"MLK3Tools: Failed to fetch fixes index: {exc}")
...
def check_for_fixes(appid: int) -> str:
    ...
    try:
        ...
        logger.warn(f"MLK3Tools: Failed to fetch game name for {appid}: {exc}")
    ...
    if index is not None:
        generic_url = f"https://files.mlk3tools.work/GameBypasses/{appid}.zip"
        online_url = f"https://files.mlk3tools.work/OnlineFix1/{appid}.zip"
        ...
        logger.log(f"MLK3Tools: Fix check for {appid} via index: generic={appid in index['generic']}, online={appid in index['online']}")
    else:
        # Fallback: HEAD requests if index is unavailable
        logger.warn(f"MLK3Tools: Fixes index unavailable, falling back to HEAD requests for {appid}")
        client = ensure_http_client("MLK3Tools: CheckForFixes")
        try:
            generic_url = f"https://files.mlk3tools.work/GameBypasses/{appid}.zip"
            ...
            logger.warn(f"MLK3Tools: Generic fix check failed for {appid}: {exc}")
        ...
        try:
            ...
            logger.warn(f"MLK3Tools: Online-fix check failed for {appid}: {exc}")

    return json.dumps(result)


def _download_and_extract_fix(appid: int, download_url: str, install_path: str, fix_type: str, game_name: str = ""):
    client = ensure_http_client("MLK3Tools: fix download")
    try:
        ...
        logger.log(f"MLK3Tools: Downloading {fix_type} from {download_url}")

        with client.stream("GET", download_url, follow_redirects=True, timeout=30) as resp:
            logger.log(f"MLK3Tools: Fix download response for {appid}: status={resp.status_code}")
            ...
            with open(dest_zip, "wb") as output:
                for chunk in resp.iter_bytes():
                    ...
                    if state.get("status") == "cancelled":
                        logger.log(f"MLK3Tools: Fix download cancelled before writing chunk for {appid}")
                    ...
                    if _get_fix_download_state(appid).get("status") == "cancelled":
                        logger.log(f"MLK3Tools: Fix download cancelled for {appid}")
                        raise RuntimeError("cancelled")

        logger.log(f"MLK3Tools: Download complete, extracting to {install_path}")
        ...
        with zipfile.ZipFile(dest_zip, "r") as archive:
            ...
            if _get_fix_download_state(appid).get("status") == "cancelled":
                logger.log(f"MLK3Tools: Fix extraction cancelled before start for {appid}")
                raise RuntimeError("cancelled")

            if len(top_level_entries) == 1 and appid_folder.rstrip("/") in top_level_entries:
                logger.log(f"MLK3Tools: Found single folder {appid} in zip, extracting its contents")
                for member in archive.namelist():
                    if member.startswith(appid_folder) and member != appid_folder:
                        ...
                        if not _is_safe_path(install_path, target_path):
                            logger.warn(f"MLK3Tools: Skipping potentially unsafe path in zip: {member}")
                            continue
                        ...
                        source.close()
                        if _get_fix_download_state(appid).get("status") == "cancelled":
                            logger.log(f"MLK3Tools: Fix extraction cancelled mid-process for {appid}")
                            raise RuntimeError("cancelled")
            else:
                logger.log(f"MLK3Tools: Extracting all zip contents to {install_path}")
                for member in archive.namelist():
                    if member.endswith("/"):
                        continue
                    # Validate path doesn't escape install directory (prevent path traversal)
        ...
        if _get_fix_download_state(appid).get("status") == "cancelled":
            logger.log(f"MLK3Tools: Fix cancelled after extraction for {appid}")
            raise RuntimeError("cancelled")
                ...
                if ini_relative_path:
                    ...
                    if os.path.exists(ini_full_path):
                        ...
                        if updated_contents != contents:
                            ...
                            logger.log(f"MLK3Tools: Updated unsteam.ini with appid {appid}")
                        else:
                            logger.log("MLK3Tools: unsteam.ini did not contain <appid> placeholder or was already updated")
                    else:
                        logger.warn(f"MLK3Tools: Expected unsteam.ini at {ini_full_path} but file not found")
                else:
                    logger.warn("MLK3Tools: Extracted files do not include unsteam.ini for Online Fix (Unsteam)")
            except Exception as exc:
                logger.warn(f"MLK3Tools: Failed to update unsteam.ini: {exc}")

        log_file_path = os.path.join(install_path, f"mlk3tools-fix-log-{appid}.log")
        try:
            ...
            logger.log(f"MLK3Tools: Appended fix log at {log_file_path} with {len(extracted_files)} files")
        except Exception as exc:
            logger.warn(f"MLK3Tools: Failed to create fix log file: {exc}")

        logger.log(f"MLK3Tools: {fix_type} applied successfully to {install_path}")
        ...
        if str(exc) == "cancelled":
            ...
            return
        logger.warn(f"MLK3Tools: Failed to apply fix: {exc}")
...
def apply_game_fix(appid: int, download_url: str, install_path: str, fix_type: str = "", game_name: str = "") -> str:
    ...
    logger.log(f"MLK3Tools: ApplyGameFix appid={appid}, fixType={fix_type}")
...
def cancel_apply_fix(appid: int) -> str:
    ...
    _set_fix_download_state(appid, {"status": "cancelled", "success": False, "error": "Cancelled by user"})
    logger.log(f"MLK3Tools: CancelApplyFix requested for appid={appid}")
    return json.dumps({"success": True})


def _unfix_game_worker(appid: int, install_path: str, fix_date: str = None):
    try:
        logger.log(f"MLK3Tools: Starting un-fix for appid {appid}, fix_date={fix_date}")
        log_file_path = os.path.join(install_path, f"mlk3tools-fix-log-{appid}.log")
        ...
        try:
            ...
            logger.log(f"MLK3Tools: Found {len(files_to_delete)} unique files to remove from log")
        except Exception as exc:
            logger.warn(f"MLK3Tools: Failed to read log file: {exc}")
        ...
        for file_path in files_to_delete:
            try:
                ...
                if os.path.exists(full_path):
                    os.remove(full_path)
                    deleted_count += 1
                    logger.log(f"MLK3Tools: Deleted {file_path}")
            except Exception as exc:
                logger.warn(f"MLK3Tools: Failed to delete {file_path}: {exc}")

        logger.log(f"MLK3Tools: Deleted {deleted_count}/{len(files_to_delete)} files")
        ...
        if remaining_fixes:
            ...
            try:
                ...
                logger.log(f"MLK3Tools: Updated log file, {len(remaining_fixes)} fixes remaining")
            except Exception as exc:
                logger.warn(f"MLK3Tools: Failed to update log file: {exc}")
        else:
            # No fixes remaining, delete the log file
            try:
                os.remove(log_file_path)
                logger.log(f"MLK3Tools: Deleted log file {log_file_path}")
            except Exception as exc:
                logger.warn(f"MLK3Tools: Failed to delete log file: {exc}")

        _set_unfix_state(appid, {"status": "done", "success": True, "filesRemoved": deleted_count})

    except Exception as exc:
        logger.warn(f"MLK3Tools: Un-fix failed: {exc}")
...
def unfix_game(appid: int, install_path: str = "", fix_date: str = "") -> str:
    ...
    logger.log(f"MLK3Tools: UnFixGame appid={appid}, path={resolved_path}, fix_date={fix_date}")
...
def get_installed_fixes() -> str:
    """Scan all Steam library folders for games with mlk3tools fix logs."""
    try:
        ...
        try:
            ...
            logger.warn(f"MLK3Tools: Failed to parse libraryfolders.vdf: {exc}")
            ...
            try:
                for filename in os.listdir(steamapps_path):
                    ...
                    try:
                        ...
                        # Check for mlk3tools fix log
                        log_file_path = os.path.join(full_install_path, f"mlk3tools-fix-log-{appid}.log")
                        if os.path.exists(log_file_path):
                            ...
                            try:
                                ...
                                logger.warn(f"MLK3Tools: Failed to parse fix log for {appid}: {exc}")

                    except Exception as exc:
                        logger.warn(f"MLK3Tools: Failed to process manifest {filename}: {exc}")
                        continue

            except Exception as exc:
                logger.warn(f"MLK3Tools: Failed to scan library {lib_path}: {exc}")
                continue

        return json.dumps({"success": True, "fixes": installed_fixes})

    except Exception as exc:
        logger.warn(f"MLK3Tools: Failed to get installed fixes: {exc}")
