...
def _copy_webkit_files() -> None:
    ...
    logger.log(f"Copying MLK3Tools web UI from {js_src} to {js_dst}")
    try:
        shutil.copy(js_src, js_dst)
    except Exception as exc:
        logger.error(f"Failed to copy MLK3Tools web UI: {exc}")
    ...
    if os.path.exists(icon_src):
        try:
            ...
            logger.log(f"Copied MLK3Tools icon to {icon_dst}")
        except Exception as exc:
            logger.error(f"Failed to copy MLK3Tools icon: {exc}")
    else:
        logger.warn(f"MLK3Tools icon not found at {icon_src}")
...
def _inject_webkit_files() -> None:
    ...
    logger.log(f"MLK3Tools injected web UI: {js_path}")
...
def HasMLK3ToolsForApp(appid: int, contentScriptQuery: str = "") -> str:
    return has_mlk3tools_for_app(appid)


def StartAddViaMLK3Tools(appid: int, contentScriptQuery: str = "") -> str:
    return start_add_via_mlk3tools(appid)


def GetAddViaMLK3ToolsStatus(appid: int, contentScriptQuery: str = "") -> str:
...
def CancelAddViaMLK3Tools(appid: int, contentScriptQuery: str = "") -> str:
    return cancel_add_via_mlk3tools(appid)
...
def GetMorrenusStats(api_key: str, force_refresh: bool = False, contentScriptQuery: str = "", **kwargs: Any) -> str:
    ...
    try:
        ...
        client = ensure_http_client("MLK3Tools: GetMorrenusStats")
        ...
        logger.warn(f"MLK3Tools: GetMorrenusStats failed: {exc}")
        return json.dumps({"error": str(exc)})


def StartAddViaMLK3ToolsFromUrl(appid: int, url: str, apiName: str, contentScriptQuery: str = "") -> str:
    return start_add_via_mlk3tools_from_url(appid, url, apiName)
...
def DeleteMLK3ToolsForApp(appid: int, contentScriptQuery: str = "") -> str:
    return delete_mlk3tools_for_app(appid)
...
def GetInstalledMLK3Scripts(contentScriptQuery: str = "") -> str:
    return get_installed_mlk3_scripts()
...
def OpenExternalUrl(url: str, contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"MLK3Tools: OpenExternalUrl failed: {exc}")
...
def GetSettingsConfig(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"MLK3Tools: GetSettingsConfig failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def ApplySettingsChanges(
    ...
    try:
        ...
        try:
            logger.log(
                "MLK3Tools: ApplySettingsChanges raw argument "
                f"type={type(changes)} value={changes!r}"
            )
            logger.log(f"MLK3Tools: ApplySettingsChanges kwargs: {kwargs}")
        except Exception:
            pass
        ...
        if isinstance(changes, str) and changes:
            try:
                ...
                logger.warn("MLK3Tools: Failed to parse changes string payload")
                ...
                if isinstance(payload, dict) and "changes" in payload:
                    ...
                    try:
                        ...
                        logger.warn("MLK3Tools: Failed to parse changesJson string inside payload")
                        return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if "changesJson" in changes and isinstance(changes["changesJson"], str):
                try:
                    ...
                    logger.warn("MLK3Tools: Failed to parse changesJson payload from dict")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if isinstance(changes_json, dict):
                ...
                try:
                    ...
                    logger.warn("MLK3Tools: Failed to parse changesJson payload")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            else:
                payload = changes
        ...
        if payload is None:
            ...
            logger.warn(f"MLK3Tools: Parsed payload is not a dict: {payload!r}")
            return json.dumps({"success": False, "error": "Invalid payload format"})

        try:
            logger.log(f"MLK3Tools: ApplySettingsChanges received payload: {payload}")
        except Exception:
            pass

        result = apply_settings_changes(payload)
        try:
            logger.log(f"MLK3Tools: ApplySettingsChanges result: {result}")
        except Exception:
            pass
        response = json.dumps(result)
        try:
            logger.log(f"MLK3Tools: ApplySettingsChanges response json: {response}")
        except Exception:
            pass
        return response
    except Exception as exc:
        logger.warn(f"MLK3Tools: ApplySettingsChanges failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableLocales(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"MLK3Tools: GetAvailableLocales failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetTranslations(contentScriptQuery: str = "", language: str = "", **kwargs: Any) -> str:
    try:
        ...
        logger.warn(f"MLK3Tools: GetTranslations failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableThemes(contentScriptQuery: str = "") -> str:
    ...
    try:
        ...
        logger.warn(f"MLK3Tools: GetAvailableThemes failed: {exc}")
...
class Plugin:
    ...
    def _load(self):
        logger.log(f"bootstrapping MLK3Tools plugin, millennium {Millennium.version()}")

        try:
            detect_steam_install_path()
        except Exception as exc:
            logger.warn(f"MLK3Tools: steam path detection failed: {exc}")
        ...
        try:
            ...
            logger.warn(f"MLK3Tools: settings initialization failed: {exc}")
        ...
        try:
            ...
            logger.warn(f"MLK3Tools: Fixes index initialization failed: {exc}")
